...
public class SoundManager : MonoBehaviour
{
    ...
    private void Start()
    {
        mixer = (AudioMixer)Resources.Load("AudioMixer/Main Mixer");
        if (!mixer)
        {
    ...
    public void SetVolume(SoundType type, float Slidervolume)
    {
        mixer.SetFloat("Volume" + type.ToString(), Mathf.Log(Slidervolume) * 20.0f);
    }
    ...
    public void Play(string clipname, SoundType type, bool loop)
    {
        ...
        // load the clip
        AudioClip newClip = (AudioClip)Resources.Load(type.ToString() + "/" + clipname);
        // Now Play it!
